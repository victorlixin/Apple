...
public:
    ...
    _CRTIMP int __thiscall operator==(__in const type_info& _P(_Rhs)) const;
    _CRTIMP int __thiscall operator!=(__in const type_info& _P(_Rhs)) const;
    _CRTIMP int __thiscall before(__in const type_info& _P(_Rhs)) const;
#ifdef _SYSCRT
...
private:
    void *_m_data;
    char _m_d_name[1];
    __thiscall type_info(__in const type_info& _P(_Rhs));
    type_info& __thiscall operator=(__in const type_info& _P(_Rhs));
};
#ifndef _TICORE
...
public:
    ...
    __CLR_OR_THIS_CALL bad_cast(__in const bad_cast & _P(_Bad_cast));
...
private:
    ...
    bad_cast(__in_z const char * const * _P(_Message));
#endif  /* CRTDLL */
...
public:
    __non_rtti_object(__in_z const char * _P(_Message));
