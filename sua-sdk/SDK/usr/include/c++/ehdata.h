...
#define EH_MAGIC_NUMBER3        0x19930522  // latest magic #. Note that We
                                            // can't use top 3 bits and we can't also decrease
                                            // this number Because older CRT versions do
                                            // less then check to check if it can handle this
                                            // version. This leaves us with incrementing the
                                            // version just by one.
...
typedef const struct _s_HandlerType {
	...
	__int32			frameNestLevel;		// The static nesting level of parent function
#endif
...
//
typedef unsigned __int32 EHRegistrationNode;
...
typedef struct _xDISPATCHER_CONTEXT {
    __int32 ControlPc;
    PRUNTIME_FUNCTION FunctionEntry;
    unsigned __int32 EstablisherFrame;
    PCONTEXT ContextRecord;
} DispatcherContext;
...
typedef struct _ftinfo {
    unsigned __int32 dwMagicNumber;                // magic number
    void *pFrameInfo;			// pointer to runtime frame info
    PRUNTIME_FUNCTION rgFuncTable;	// function table
    unsigned __int32 cFuncTable;			// number of function entry
    unsigned __int32 dwEntryCF;			// address of starting of the code fragment
    unsigned __int32 dwSizeCF;			// size of the code fragment
} FTINFO, *PFTINFO;
